...
describe('rv-task-results routes', function() {
  ...
  describe('get /rv-task-results/', function() {
    ...
    it('it responds with 400 when there is some exception ' +
      'trying to get the rv task results', function(done){
      var stub = sinon.stub(RvTaskResult, 'findAll').rejects(
  ...
  describe('post /rv-task-results', function() {
    it('when there is already an active task resutl, ' +
      'it updates the existing task result', function(done){
      var taskResult = [
        [
          {
            userId: 1,
    ...
    it('it responds with 400 when there is some exception trying to update a task', function(done){
      var stub = sinon.stub(RvTaskResult, 'upsert').rejects(new Error('error in upsert function'));
