...
describe('rv-remuneration-point routes', function() {
  ...
  describe('get /rv-remuneration-points/', function() {
    it('it returns the active remuneration-points', function(done) {
      var rvRemunerationPoints = {
        rv_remuneration_points: [
          {
            id: rvRemunerationPoint1.id,
            value: rvRemunerationPoint1.value,
            minPoints: rvRemunerationPoint1.minPoints,
          },
          {
            id: rvRemunerationPoint2.id,
            value: rvRemunerationPoint2.value,
            minPoints: rvRemunerationPoint2.minPoints,
          },
        ],
      };
      supertest(app)
    ...
    it('it responds with 400 when there is some exception' +
      ' trying to get the active remuneration points', function(done){
      var stub = sinon.stub(RvRemunerationPoint, 'findAll').rejects(
  ...
  describe('post /rv-remuneration-points/', function() {
    ...
    it('it fails to create a new rv remuneration when the minimum punctuation is negative',
      function(done){
        supertest(app)
          .post('/rv-remuneration-points/')
          .send({
    ...
    it('it fails to create a new rv remuneration when the same ' +
      'punctuation are inserted more than once.', function(done){
      supertest(app)
        .post('/rv-remuneration-points/')
        .send({
          rv_remuneration_points: [{
    ...
    it('it fails to create a new rv remuneration when the same ' +
      'values are inserted more than once.', function(done){
      supertest(app)
        .post('/rv-remuneration-points/')
        .send({
          rv_remuneration_points: [{
