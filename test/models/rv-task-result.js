...
describe('rv-task-results', function() {
  ...
  it('it return the active task results', function() {
    return RvTaskResult.scope(
      ...
      var dataValues = taskResults.map(function(taskResult) {
  ...
  it('it sets the base date to the last day of the month before save', function() {
    date = new Date();
    expect(new Date(taskResult1.baseDate).setHours(0, 0, 0, 0)).to.eq(
      new Date(date.getFullYear(), date.getMonth() + 1, 0).setHours(0, 0, 0, 0),
    );
  });
  it('it throws an exception when we try to create a task result for a base date ' +
    'that already exists', function() {
    var promise = RvTaskResult.create({
      userId: 1,
      taskNumber: 1,
      points: 15,
      taskValue: 30,
