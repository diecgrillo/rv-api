...
describe("rv-goal-range-param", function() {
  ...
  it("it return the actives rv goal range params", function() {
    return RvGoalRangeParam.scope('actives').findAll({}).then((rvGoalRangeParams) => {
      dataValues = rvGoalRangeParams.map(function(rvGoalRangeParam) {
        return rvGoalRangeParam.dataValues
      });
      expect(dataValues).to.have.deep.members([rvGoalRangeParam1,rvGoalRangeParam2])
    });
  });
  it("it throws an exception when we try to create more than one rv goal range param with same percentage", function() {
    promise = RvGoalRangeParam.bulkCreate([
      {
        percentage: 1.15,
        startDate: new Date(),
        endDate: null
    ...
    return promise.should.be.rejectedWith(Error, 'Cannot insert the same percentage more than once.');
  });
  it("it throws an exception when we try to create a rv goal range param with values that are already active", function() {
    promise = RvGoalRangeParam.bulkCreate([{
      percentage: 0.95,
      startDate: new Date(),
      endDate: null
  ...
  it("it throws an exception when we try to create a rv goal range param with negative percentage", function() {
    promise = RvGoalRangeParam.create({
      percentage: -1.1,
      startDate: new Date(),
      endDate: null
    });
