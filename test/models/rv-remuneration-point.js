...
describe('rv-remuneration-point', function() {
  var date = new Date();
  var rvRemunerationPoint1, rvRemunerationPoint2;
  ...
  it('it return the active rv remuneration points', function() {
    return RvRemunerationPoint.scope('actives').findAll({}).then((rvRemunerationPoints) => {
      var dataValues = rvRemunerationPoints.map(function(rvRemunerationPoint) {
        return rvRemunerationPoint.dataValues;
      });
      expect(dataValues).to.have.deep.members([rvRemunerationPoint1, rvRemunerationPoint2]);
    });
  });
  it('it throws an exception when we try to create more than one rv remuneration ' +
    'point with same points', function() {
    var promise = RvRemunerationPoint.bulkCreate([
      {
        minPoints: 1800,
        value: 1.15,
        startDate: new Date(),
    ...
    return promise.should.be.rejectedWith(
      Error, 'Cannot insert the same punctuation more than once.',
    );
  });
  it('it throws an exception when we try to create a rv remuneration point with ' +
  ...
  it('it throws an exception when we try to create a rv remuneration point with ' +
    ...
    return promise.should.be.rejectedWith(
      Error, 'There is already a remuneration active for the provided points or values: ' +
  ...
  it('it throws an exception when we try to create a rv remuneration point with ' +
    'negative points', function() {
    var promise = RvRemunerationPoint.create({
      minPoints: -1700,
      value: 1.15,
      startDate: new Date(),
      endDate: null,
    ...
    return promise.should.be.rejectedWith(
      Error, 'Validation error: Validation min on minPoints failed',
    );
  });
  it('it throws an exception when we try to create a rv remuneration point with ' +
    ...
    return promise.should.be.rejectedWith(
      Error, 'Validation error: Validation min on value failed',
    );
  });
});
