...
describe('rv-results', function() {
  ...
  it('it return the active task results', function() {
    return RvResult.scope(
      ...
      var dataValues = results.map(function(result) {
  ...
  it('it sets the base date to the last day of the month before save', function() {
    date = new Date();
    expect(new Date(result1.baseDate).setHours(0, 0, 0, 0)).to.eq(
      new Date(date.getFullYear(), date.getMonth() + 1, 0).setHours(0, 0, 0, 0),
    );
  });
  it('it throws an exception when we try to create a rasult for a base date that ' +
    'already exists', function() {
    var promise = RvResult.create({
      userId: 1,
      points: 15,
      value: 300.00,
      origination: 100000,
  ...
  it('it throws an exception when we try to create a result with negative percentage', function() {
    ...
    return promise.should.be.rejectedWith(
      Error, 'Validation error: Validation min on remuneration failed',
    );
  });
});
